...
SwitchActor.prototype.draw = function() {
    ...
    if(this.activated == false) {
	 	var prop = {fill:false, color:this.getLaserColor(this,this.switchColorNum,'halo'),width:5};     
	    prop.source = "default";
	    prop.writeTo = 2;
    ...
    for(var i in this.switchTargets) {
	    ...
	    if(this.activated == false)		prop.color = this.getLaserColor(this,this.switchColorNum,'halo');
    ...
    if(this.activated == false) {
	    for(var i in this.laserTouches) {
			var lColor = this.getLaserColor(this, this.laserTouches[i], 'switch');
...
SwitchActor.prototype.addTouchLaser = function(num) {
	var Llist = this.getLaserParts(this, num);
	for(var i in Llist) {
		var cnum = Llist[i];
...
SwitchActor.prototype.update = function() {
	...
	this.switchColor = this.getLaserColor(this, this.switchColorNum, 'switch');
	this.laserComponents = this.getLaserParts(this, this.switchColorNum);
